...
Order lines are nested within orders and don't have separate endpoints. Each line item references a variant partnership price (the price of a product variant for a specific partnership) and includes quantity, pricing, and optional properties.
...
3. **By Description** - If the line includes a `description`, it matches case-insensitively with existing lines
4. **Create** - Lines without a match are created as new
5. **Delete** - Existing lines without a match in the request are deleted
...
POST /orders
{
  ...
  "lines": [
    {
      "variantPartnershipPriceId": "vpp_123",
      "quantity": 20,
      "unitPrice": 75.00
    }
  ]
...
PATCH /orders/ord_12345
{
  "lines": [
    ...
    {
      "variantPartnershipPriceId": "vpp_789",  // New line
      "quantity": 3,
      "unitPrice": 50.00
    }

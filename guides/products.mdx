...
- `name` - Variant identifier (unique within product)
- `uom` - Unit of measure
- `defaultPrice` - Default pricing
...
- Weights: `netWeightKg`, `grossWeightKg`
- `quantityInBaseUom` - Quantity in base unit
...
1. **By ID (Priority 1)** - If an incoming variant includes an `id` field, it matches directly to an existing variant
...
Update specific variants using their IDs:
...
<Tip>
**Variant IDs**: When tracking variants, store their IDs for reliable updates. This avoids potential name-matching issues.
</Tip>

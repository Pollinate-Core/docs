...
```bash Production
curl -X POST https://www.api.pollinate.tech/token \
...
const response = await fetch('https://www.api.pollinate.tech/token', {
  method: 'POST',
  headers: {
...
response = requests.post(
    'https://www.api.pollinate.tech/token',
...
<Warning>
Store your token securely and never commit it to version control. Tokens expire after a period of inactivity - you'll need to request a new token when this happens.
</Warning>
...
```bash Production
curl -X GET https://www.api.pollinate.tech/products \
...
```javascript Production
const response = await fetch('https://www.api.pollinate.tech/products', {
  headers: {
...
response = requests.get('https://www.api.pollinate.tech/products', headers=headers)
```
...
<Tip>
**Token Management**: Implement token refresh logic to automatically request a new token when you receive a 401 response. Store tokens securely and avoid hardcoding them in your application.
</Tip>
